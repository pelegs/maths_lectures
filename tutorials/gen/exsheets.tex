...
\newcommand{\draw}[1]{
	\begin{tabular}{c}
		$\pyc{draw_matrix(#1)}$\\[7mm]
		(#1)
	\end{tabular}
}

\newcommand{\drawtranspose}[1]{
	\begin{tabular}{c}
		$\pyc{draw_matrix(#1.T)}$\\[7mm]
		(#1)
	\end{tabular}
}

\newcommand{\drawaddition}[2]{
	\begin{tabular}{c}
		$\pyc{draw_add(#1,#2)}$\\[7mm]
		(#1+#2)
	\end{tabular}
}

\newcommand{\drawprod}[2]{
	\begin{tabular}{c}
		$\pyc{draw_prod(#1,#2)}$\\[7mm]
		($#1\cdot#2$)
	\end{tabular}
}
...
\title{Computer Science and Mathematics (B.MES.108)\\Summer Semester 2020\\}
\author{Peleg Bar Sapir}
 
\def\exnum{\ex}
\setcounter{secnumdepth}{2}
\renewcommand{\thesection}{Exercise Sheet\ex:}
\renewcommand{\thesubsection}{Problem \arabic{subsection}:}
\titleformat{\section}[display]{\normalfont\bfseries\Large}{Exercise \ex: #1\solutionhead}{-20pt}{}

\begin{document}
\rhead{Exercise Sheet \ex\solutionhead}
\include{exsheets/exsheet\ex}
\newpage
\end{document}
